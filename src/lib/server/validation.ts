...
export const applicationSchema = z.object({
  ...
  phone: z.string().regex(/^\+?\d[\d\s().-]{8,15}$/, "Please enter a valid phone number"),
  dateOfBirth: z.string().min(1, "Date of birth is required"),
  ssn: z.string().min(1, "This field is required").refine(
    (val) =>
  ...
  zipCode: z.string().regex(/^(\d{5}(-\d{4})?|[A-Za-z]\d[A-Za-z]\s?\d[A-Za-z]\d|\d{6})$/, "Invalid postal code"),
  ...
  loanPurpose: z.enum(["debt-consolidation", "home-improvement", "medical", "auto", "business", "education", "other"]),
  loanTerm: z.number().refine((v) => [12, 24, 36, 48, 60].includes(v), { message: "Invalid loan term" }),
  routingNumber: z.string().min(1).refine(
